...
@section Title
{<![CDATA[
	<a href="Add" class="Button" data-options='{ "icons": { "primary": "ui-icon-plus" } }'>New ]]></xsl:text>
    	<xsl:value-of select="$displayName"/>
    	<xsl:text><![CDATA[</a> @this.ViewBag.Title]]>
}
...
@{Html.RenderPartial("Table", new TableModel(Url.Action("Edit", new { ID = "{ID}", ReturnUrl = this.Request.Url.ToString() }), this.Model.Items));}
</xsl:text>	
	</xsl:template>
</xsl:stylesheet>